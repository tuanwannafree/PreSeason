...
#include <string>
#include <vector>
#include <stack>     // <<< THÊM: Để dùng std::stack<char> cho toán tử
#include <cmath>
#include <cctype>
#include <stdexcept>
#include <climits>
...
// --- Giả định các hàm tính toán số lớn (Cần cài đặt chi tiết) ---
// (Giữ nguyên phần giả định như trước, vẫn dùng stoll tạm thời)
...
int BigIntCompare(string num1, string num2) { /* ... (Phần giả định như trước) ... */ if (num1.length() > num2.length()) return 1; if (num1.length() < num2.length()) return -1; return num1.compare(num2); }
// --- Kết thúc phần giả định ---


// --- Stack Danh Sách Liên Kết Tùy Chỉnh (cho số lớn dạng string) ---
// --- Node bây giờ lưu trữ string ---
...
typedef struct tagList
{
    Node *pHead;
    Node *pTail;
} LIST; // Stack này dùng cho số lớn (toán hạng) trong calculate

// Các hàm CreateList, CreateNode(string), IsEmpty, Push(Node*), Pop, Top(string) giữ nguyên như phiên bản trước
...
// --- Kết thúc Stack Implementation ---


// Hàm Vorcedence (pri) giữ nguyên
int pri(char a) { if(a=='+'||a=='-') return 1; if(a=='*'||a=='/') return 2; if(a=='^') return 3; return 0; }

// --- Hàm convert (Chuyển Infix sang Postfix) ---
// <<< SỬA ĐỔI: Dùng std::stack<char> cho toán tử >>>
string convert(string infix)
{
    int i = 0;
    string postfix = "";
    stack<char> s_operator; // <<< THAY ĐỔI: Dùng stack<char> của thư viện chuẩn
    ...
    while (infix[i] != '\0')
    {
        ...
        {
            s_operator.push(infix[i]); // <<< THAY ĐỔI: Dùng push của std::stack
            i++;
        }
        else if (infix[i] == ')')
        {
            while (!s_operator.empty() && s_operator.top() != '(') // <<< THAY ĐỔI: Dùng empty() và top()
            {
                postfix += s_operator.top(); // <<< THAY ĐỔI: Lấy char từ top()
                postfix += ' ';
                s_operator.pop(); // <<< THAY ĐỔI: Dùng pop()
            }
            if (s_operator.empty()) { return "[Error: Mismatched )]"; }
            s_operator.pop(); // Pop dấu '('
            i++;
        }
        else if (pri(infix[i]) > 0) // Operator
        {
            ...
            s_operator.push(infix[i]); // <<< THAY ĐỔI: Dùng push()
            i++;
        }
        else
        {
...
// --- Hàm calculate (Tính giá trị Postfix) ---
// Sử dụng stack LIST chứa string số lớn (Giữ nguyên như trước)
string calculate(string postfix)
{
    LIST s_operand; // Stack cho toán hạng (số lớn dạng string)
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        {
             // Kiểm tra số hợp lệ trước khi push (ví dụ)
            try {
                // Thử chuyển đổi tạm thời để bắt lỗi định dạng cơ bản, dù stoll sẽ sai với số lớn
                 stoll(curItem);
        ...
        else if (curItem.length() == 1 && pri(curItem[0]) > 0) // Operator
        {
            ...
            switch (curItem[0])
            {
             ...
             // Kiểm tra lỗi trả về từ hàm BigInt
...
// --- Biến và Hàm cho Bộ Nhớ (Giữ nguyên như trước) ---
...
int main() {
    ...
    while (true) {
        cout << "> ";
        if (!getline(cin, input_line)) break; // Thoát nếu có lỗi đọc input
        ...
        if (input_line == "quit") {
            ...
            // TODO: Xử lý thay thế "MR" bằng memoryValue nếu muốn dùng trong biểu thức
            // Ví dụ đơn giản: (cần xử lý kỹ hơn các trường hợp phức tạp)
            ...
            if (postfix.find("[Error:") == string::npos) {
                ...
                cout << "  Result: " << result << endl;
                 // Chỉ lưu kết quả hợp lệ vào lastResult
